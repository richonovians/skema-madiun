...
async function main(): Promise<void> {
  ...
  console.log(
    `Seed selesai: admin kabupaten (id=${adminKabupaten.id}, = superuser), admin OPD (id=${adminOpd.id}), responden (id=${responden.id}), ${opdSeed.length} OPD, template ${SKM_UNSUR.length} unsur (2 survei: 1 draft + 1 aktif), 1 pengaduan contoh.`,
  );
}
