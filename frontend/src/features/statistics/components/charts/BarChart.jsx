          ...
          return (
            <div key={index} className="flex flex-col gap-1 group">
              <div className="flex justify-between text-sm">
                <span className="font-medium text-text-secondary group-hover:text-primary transition-colors">{item.category || item.label}</span>
                <span className="font-bold text-text-primary">{item.count || `${item.percentage}%`}</span>
              </div>
