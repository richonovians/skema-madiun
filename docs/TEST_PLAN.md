...
| **Cleanup**       | **Direvisi 15 Sep 2026 — pernyataan lama ("setiap suite membersihkan data setelah selesai, transactional rollback atau truncate") TIDAK BENAR untuk lapisan E2E.** Suite Playwright berjalan melawan backend & basis data sungguhan lewat HTTP; ia tak punya transaksi untuk digulung balik, dan aplikasinya sendiri tak menyediakan penghapusan (pengaduan tanpa endpoint `DELETE`; survei hanya terhapus saat `draft`). Tiap jalannya meninggalkan ± 1 pengaduan, 2 respons, dan beberapa notifikasi. Sebelas hari tanpa pembersihan menghasilkan **7.986 notifikasi tanpa induk** di lonceng akun admin sungguhan. Pembersihan karena itu **eksplisit, bukan otomatis**: jalankan `node apps/web/e2e/support/bersihkan-data-uji.mjs --tanpa-induk` (`--dry` untuk melihat lebih dulu). **Tiap kali sesudah menjalankan apa pun yang menulis ke basis data — termasuk satu berkas spec saja — bukan sekali di akhir sesi** (permintaan tersurat pengguna, 3 September dan ditegaskan ulang 15 September 2026). Bendera `--tanpa-induk` hampir selalu perlu: penyaring bawaan bertolak dari survei yang MASIH ADA, sedangkan pagar regresi BUG-014 memusnahkan surveinya sendiri, jadi tiap jalannya meninggalkan lima notifikasi yang tak terbaca sebagai data uji. **Garis dasar `skm_db` per 15 September 2026** — cocokkan sesudah membersihkan: 13 pengaduan · 5 survei · 5 respons · 7 pengguna · 62 OPD · 265 notifikasi, dan `/statistics` menunjukkan IKM 82,64 dengan 5 responden. Klaim lama tetap berlaku bagi integration test backend yang memakai basis data ephemeral. |
...
- [ ] Seed data tersedia dan dapat dijalankan. Sempat tak terpenuhi 5–15 Sep 2026, lihat [CAT-014](BUG_REPORTS.md#cat-014). **Dari mana dijalankan menentukan** (terpantau 15 Sep 2026):
  ...
  > ⚠️ **Lingkungan hasil seed TIDAK dapat menguji peran jamak** —
  > [CAT-021](BUG_REPORTS.md#cat-021). Keempat akun seed berperan tunggal,
  > sehingga `actingRole: null`, 401 "Peran yang ingin dipakai belum dipilih",
  > `POST /auth/acting-role`, pemilih peran, dan **seluruh charter C-15** tak
  > pernah dapat diamati di sana. Kelima akun berperan jamak di `skm_db` disetel
  > dengan tangan dan penyetelannya tak tercatat di repositori mana pun.

- [ ] **Titik awal yang dapat direproduksi — apa yang sebenarnya bergantung padanya.**
  Angka Jest (**673/673 di 92 berkas**) **tidak** menyentuh basis data sama
  sekali: MSW yang menjawab setiap permintaan. Ia sudah dapat direproduksi di
  mesin mana pun tanpa seed, tanpa Docker, tanpa server dev. Yang bergantung pada
  basis data hanya lapisan E2E — dan ketakstabilannya berasal dari sisi peramban
  ([TEST_CASES §Y.3](TEST_CASES.md)), bukan dari isi basis datanya. Menjalankan
  ulang seluruh suite di atas basis data hasil seed karena itu **tidak** membuat
  angkanya lebih dapat dipercaya; ia hanya memindahkannya ke dunia yang berbeda
  (lihat CAT-021). Diputuskan 15 September 2026 untuk tidak menempuhnya.
